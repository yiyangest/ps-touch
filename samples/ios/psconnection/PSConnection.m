...
- (NSUInteger) sendNetworkPacket:(PSNetworkPacket*)packet withData:(NSData*)data
{
	...
	// allocate a buffer for the packet and data
	NSMutableData* buffer = [NSMutableData dataWithBytes:packet length:sizeof(PSNetworkPacket)];
	...
	NSUInteger length = buffer.length - sizeof(packet->status);
	...
	bytesToEncrypt = (void*)(((char*) bytesToEncrypt) + 4);
	...
	// send the buffer
	[_connection sendNetworkData:buffer];
...
- (NSUInteger) sendImage:(UIImage*)image encoding:(PSNetImageEncoding)encoding
{
	...
	{
		// allocate a buffer big enough for the image header
		// and all the image bytes, note that PS only supports
		// non-alpha raw images so RGB only (channel count = 3)
		NSUInteger length = sizeof(PSRawImage);
		length += image.size.width * image.size.height * 3;
		NSMutableData* imageData = [NSMutableData dataWithLength:length];
		void* imageDataBytes = (void*)(((char*)([imageData mutableBytes])) + sizeof(PSRawImage));
		
		// fill the raw image header
		PSRawImage* imageHeader = (PSRawImage*)[imageData mutableBytes];
		imageHeader->format = 2;	// raw pixmap
		imageHeader->width = htonl((NSUInteger)image.size.width);
		imageHeader->height = htonl((NSUInteger)image.size.height);
		imageHeader->rowBytes = htonl((NSUInteger)(image.size.width * image.size.height * 3));
		imageHeader->mode = 1;		// RGB
		imageHeader->channels = 3;
		imageHeader->bitsPerChannel = 8;
		
		// create an RGB only context, A is ignored
		CGColorSpaceRef colorspace = CGColorSpaceCreateDeviceRGB();
		CGContextRef context =  CGBitmapContextCreate(imageDataBytes,
													  image.size.width, 
													  image.size.height, 
													  8,
													  image.size.width * 3,
